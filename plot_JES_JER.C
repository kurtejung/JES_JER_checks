		 ...
		 std::string jetType = "PF")
{
  ...
  TFile *fin   = new TFile(Form("PbPb_5p02TeV_MC_ak%s_20_eta_20.root",(algo+srad[0]+jetType).c_str()),"r");
  TFile *finPP = new TFile(Form("PP_5p02TeV_MC_ak%s_20_eta_20.root",(srad[0]+jetType).c_str()),"r");

  TH1F *hrsp [knj][ncen+1][nbins_pt];
  TH1F *hMean[knj][ncen+1], *hSigma[knj][ncen+1];
  ...
  for(int nj=0; nj<knj; nj++){
	...
	cout<<"ip = "<<ip<<endl;
	if(ic < ncen)
	  hrsp[nj][ic][ip]      = (TH1F*)fin->Get(Form("hJER_%d_pt_%d_cent%d",ptbins[ip], ptbins[ip+1],ic));
	
	if(ic == ncen)
	  hrsp[nj][ic][ip]      = (TH1F*)finPP->Get(Form("hJER_%d_pt_%d",ptbins[ip], ptbins[ip+1]));

	hrsp[nj][ic][ip]->Print("base");
	...
	//fgaus = new TF1("fgaus","gaus", mean - 2.15*hrsp[nj][ic][ip]->GetRMS(), mean + 2.15*hrsp[nj][ic][ip]->GetRMS());
	...
	// mean  = (fitstatus!=0) ? hrsp[nj][ic][ip]->GetMean()     : fgaus->GetParameter(1);
	// emean = (fitstatus!=0) ? hrsp[nj][ic][ip]->GetMeanError(): hrsp[nj][ic][ip]->GetMeanError(1);
	// sig   = (fitstatus!=0) ? hrsp[nj][ic][ip]->GetRMS()/mean : fgaus->GetParameter(2)/fgaus->GetParameter(1);
	// esig  = (fitstatus!=0) ? sqrt((pow(1/mean,2)*pow(hrsp[nj][ic][ip]->GetRMSError(),2))+(pow(-hrsp[nj][ic][ip]->GetRMS()/pow(mean,2),2)*pow(emean,2))) : sqrt((pow(1/mean,2)*pow(hrsp[nj][ic][ip]->GetRMSError(),2))+(pow(-hrsp[nj][ic][ip]->GetRMS()/pow(mean,2),2)*pow(emean,2))); 



	//FitDist(hrsp[nj][ic][ip], mean, emean, sig, esig);
	//FitGauss(hrsp[nj][ic][ip], mean, emean, sig, esig);

	// hMean [nj][ic]->SetBinContent (ip+1, roundoff(mean));
	// hMean [nj][ic]->SetBinError   (ip+1, roundoff(emean));
